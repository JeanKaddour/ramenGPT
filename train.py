...
from model import GPT, apply_normuon_variance_reduction, next_multiple_of_n, polar_express, set_flex_attention_kernel_options
    ...
    def step(self):
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                # DistAdam uses lr_mul but NOT shape_mult
                ...
                # Compute step (matching train_gpt.py DistAdam)
                denom = exp_avg_sq.sqrt().add_(eps)
                step_size = lr * (bias2 ** 0.5 / bias1)
                update = exp_avg.div(denom).mul_(step_size)

                # Cautious weight decay (from train_gpt.py DistAdam)
                # Formula: update += p * mask * eff_wd * lr  where eff_wd = lr * wd * wd_mul
...
class NorMuonSingleGPU(torch.optim.Optimizer):
    ...
    Matching train_gpt.py NorMuon behavior:
    - Uses Polar Express for orthogonalization (faster than Newton-Schulz)
    - Adds a low-rank variance estimator similar to Adafactor
    - Shape-based learning rate scaling
    - Cautious weight decay with formula: p -= p * mask * eff_wd * eff_lr
    """
    ...
    def reset(self):
        """Reset momentum buffers (useful after warmup)"""
        for group in self.param_groups:
            for p in group["params"]:
                state = self.state[p]
    ...
    def step(self):
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # Orthogonalize using Polar Express (replaces Newton-Schulz)
                g = polar_express(g)

                # Determine reduction dimension based on parameter shape
                # Gates use -1 regardless of shape (from train_gpt.py)
                ...
                # Compute effective LR with shape-based scaling (from train_gpt.py NorMuon)
                shape_mult = max(1.0, p.size(-2) / p.size(-1)) ** 0.5
                eff_lr = lr * shape_mult * getattr(p, "lr_mul", 1.0)

                # Cautious weight decay (from train_gpt.py cautious_wd_and_update_inplace)
                # Formula: p = p - p * mask * eff_wd * eff_lr - v * eff_lr
                # where eff_wd = wd_mul * weight_decay * lr
                wd_mul = getattr(p, "wd_mul", 1.0)
                eff_wd = wd_mul * wd * lr

                # Combined update matching train_gpt.py cautious_wd_and_update_inplace:
                # p.copy_(p - (p * mask * wd_factor * lr_factor) - (v * lr_factor))
                # where wd_factor=eff_wd, lr_factor=eff_lr
                mask = (g * p) >= 0
                if eff_wd != 0:
...
MuonSingleGPU = NorMuonSingleGPU
...
def run_training(config, args, code: str, detected_gpu_info: dict, run_id):
    ...
    if use_muon and len(hidden_matrix_params) > 0:
        ...
        print(f"Using Muon optimizer for {len(hidden_matrix_params)} matrix parameters")
